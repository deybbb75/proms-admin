    ...
    try {
        ...
        if($duplicate_order){
            Alert::error([
                ...
                'html'  => 'A team member with the same order already exists.',
                'path'  => $redirect_path
            ]);
        }else{
...
if (isset($_POST['Edit'])) {
    try {
       ...
       // Decrypt the id
        ...
        $duplicate_order = $db->hasDuplicate('SELECT member_order FROM tbl_member WHERE member_order = :member_order AND status = "Active" AND member_id != :member_id', ['member_order' => $_POST['member_order'], 'member_id' => $member_id]);
